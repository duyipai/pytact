...
class Pixel2GradDataset(Dataset):
    def __init__(self, csv_file, device, normalize=True):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_epochs", type=int, dest="num_epochs", default=30)
    ...
    output_path = output_path + f"/model-{dt.now().strftime('%H-%M-%S')}"
    output_model_file = output_path + ".pth"
    output_npz_file = output_path + ".npz"
    ...
    np.savez(output_npz_file, mean=dataset.mean, std=dataset.std)
